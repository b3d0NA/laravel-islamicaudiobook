<section class="w-11/12 p-5 mx-auto rounded-md main-section">
    <div class="flex items-center space-x-4 books-top md:flex-col md:items-start">
        <input wire:model.debounce.500ms="search" type="search" name="search" placeholder="Search by book name, author name" class="p-2 rounded-lg w-80 md:w-full focus:ring-2 focus:ring-blue-400 focus:outline-none focus:ring-offset-4" />
        <select wire:model.debounce.500ms="filter" class="p-2 px-4 text-gray-700 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 rounded-xl md:mt-4" name="gender">
    ...
    <div class="p-5 mt-5 bg-white rounded-lg books-section">
        <div class="flex flex-wrap items-center justify-center space-x-8 space-y-8 sm:flex-col sm:space-x-0 books">
            <div wire:loading wire:target="search" class="rounded-2xl book w-[230px] border border-gray-200 overflow-hidden hover:shadow-md transition ease-in-out max-h-[450px]">
                <div class="rounded-tr-2xl rounded-br-2xl max-h-[300px] w-[230px] overflow-hidden book-image p-1">
                    <div class="object-contain w-full transition ease-in-out bg-gray-100 cursor-pointer animate-pulse h-52 rounded-2xl">
                    </div>
                </div>
                    ...
                    @auth
                        @if (auth()->user()->group_status != 1)
                            @if($book->status == 2 && auth()->user()->paid_status == 1)
                                <a wire:click="read({{ $book->id }})" class="flex items-center justify-center w-10/12 px-2 py-2 m-auto mb-3 space-x-2 text-white transition ease-in-out bg-teal-400 rounded-xl hover:bg-teal-500 focus:ring-offset-2 focus:outline-none focus:ring-2 focus:ring-teal-500" target="_blank" href="{{ $book->read_link }}">
                            ...
                            @elseif ($book->short_link)
                                <a class="flex items-center justify-center w-10/12 px-2 py-2 m-auto mb-3 space-x-2 text-white transition ease-in-out bg-blue-300 rounded-xl hover:bg-blue-500 focus:ring-offset-2 focus:outline-none focus:ring-2 focus:ring-blue-500" target="_blank" href="{{ $book->short_link }}">
                            ...
                            @else
                                <p class="px-2 py-3 my-3 text-xs italic font-light normal-case">
                                    Please fill the
                                    <a href="{{ route('user.survey.index') }}" class="text-blue-500 hover:underline">survey form</a>
            ...
            @endforelse
            @for ($i = 1; $i <= 3; $i++) <div wire:loading class="rounded-2xl book w-[230px] border border-gray-200 overflow-hidden hover:shadow-md transition ease-in-out max-h-[450px]">
                <div class="rounded-tr-2xl rounded-br-2xl max-h-[300px] w-[230px] overflow-hidden book-image p-1">
                    <div class="object-contain w-full transition ease-in-out bg-gray-100 cursor-pointer animate-pulse h-52 rounded-2xl">
                    </div>
                </div>
