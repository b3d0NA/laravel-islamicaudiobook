    ...
    <table class="table-auto w-full border rounded-xl">
        ...
        <tbody class="text-left text-gray-600">
            ...
            <tr @class(["bg-teal-50"=> $loop->even])>
                ...
                <td class="mb-4 text-sm font-normal p-2 tracking-wider border-r border-gray-200 text-center">
                    <a target="_blank" class="text-blue-500 hover:underline normal-case" href="{{$book->read_link}}">
                ...
                <td wire:ignore.self
                    ...
                    <button wire:click.prevent="delete({{$book->id}})" href="#" class="btn-bs-danger">
                        <svg wire:loading wire:target="delete({{$book->id}})" class="animate-spin h-5 w-5 text-white"
                            xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4">
                            </circle>
                            <path class="opacity-75" fill="currentColor"
            ...
            <!-- item -->
            @empty
            <tr colspan='9'>In Shaa Allaah! Add a book. No book has been added yet</tr>
            @endforelse
        </tbody>
        <!-- end table body -->
