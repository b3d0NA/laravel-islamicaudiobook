<section x-init
    ...
    <div class="flex items-center space-x-1 navigation sm:flex-col sm:space-y-3">
        <button @click="$dispatch('open-navbar')" class="hidden md:block">
            <i
                class="text-2xl text-transparent fas fa-bars bg-clip-text bg-gradient-to-br from-blue-400 to-teal-600"></i>
        </button>
        <div class="flex items-center space-x-1 md:hidden navigation sm:flex-col sm:space-y-3">
            ...
            <a href="{{route('user.messages.index')}}"
                class="flex items-center px-3 py-2 space-x-2 text-sm text-gray-600 border rounded-lg border-stone-300 hover:bg-gradient-to-r hover:from-stone-100 hover:to-teal-50 hover:text-gray-800">
                <i
                    class="text-transparent fas fa-comment-alt-dots bg-clip-text bg-gradient-to-br from-blue-400 to-teal-600"></i>
                <span>
                    Message
                    Admin
                </span>
            ...
            <a href="{{route('user.payment.index')}}"
                class="flex items-center px-3 py-2 space-x-2 text-sm text-gray-600 border rounded-lg border-stone-300 hover:bg-gradient-to-r hover:from-stone-100 hover:to-teal-50 hover:text-gray-800">
                <i
                    class="text-transparent fas fa-money-check-alt bg-clip-text bg-gradient-to-br from-blue-400 to-teal-600"></i>
                <span>Make Payment</span>
            </a>
        </div>
    </div>
    <div @open-navbar.window="$el.classList.remove('-translate-x-full')"
        @close-navbar.window="$el.classList.add('-translate-x-full')"
        class="absolute top-0 left-0 z-30 hidden w-8/12 h-screen p-5 ease-in-out -translate-x-full rounded-lg shadow-xl bg-gradient-to-b from-sky-50 to-teal-50 md:block">
    ...
    @auth
    <div x-data="{isOpen: false}" class="relative flex items-center navigation justify-self-end w-52">
        <button @click="isOpen = true" class="flex items-center justify-center w-full">
            <p class="p-2 px-4">{{auth()->user()->name}}</p>
            <i class="text-xl fas fa-angle-down"></i>
        </button>

        <div x-cloak x-show="isOpen" x-transition.scale.origin.top @click.away="isOpen = false"
            class="absolute flex flex-col items-center p-3 px-4 space-y-3 rounded-lg shadow-md top-12 profile-options bg-stone-100">
            <div class="flex items-center justify-center space-x-3">
                <span class="text-sm text-gray-500">Group:</span>
                <span @class([' p-2 px-3 text-white rounded-lg', 'bg-teal-500'=> auth()->user()->group_status,
            ...
            <div class="flex items-center justify-center space-x-3">
                ...
                <span @class(['p-2 px-3 text-white rounded-lg', 'bg-teal-500'=> auth()->user()->paid_status,
